# v0.5.3 / v0.6.0 計画: verify workflow 化 + 辞書増強 + synonym 展開

作成日: 2026-07-19 / 現行: v0.5.2

> **版番号を 2 回繰り下げました。** 本文書が v0.5.1 と呼んでいる verify-law-ids の実装は、現在は **v0.5.3** です。
>
> - 2026-09-12: v0.5.1 を JSDoc の `@since` / `@group` 追加と README の誤り修正（公開 API 変更なし）に使用 → verify-law-ids は v0.5.2 へ
> - 2026-09-20: v0.5.2 を README の壊れたリンク修正（#5）とパッケージメタ情報の整備（#7）に使用 → verify-law-ids は v0.5.3 へ
>
> 以下の見出しと表は作成時のまま残しています。

## 0. 版付け方針（RELEASE.md の運用実績に準拠）

| 版 | 内容 | 種別 |
|---|---|---|
| ~~v0.5.1~~ | ~~verify-law-ids 本実装~~ → 繰り下げ（v0.5.1 は JSDoc + README の修正に使用） | patch |
| ~~v0.5.2~~ | ~~verify-law-ids 本実装~~ → **v0.5.3 に繰り下げ**（v0.5.2 は README リンク修正 #5 + メタ情報整備 #7 に使用） | patch |
| **v0.5.3** | verify-law-ids 本実装 + workflow 新設 + 既知の不整合修正 | patch（公開 API 変更なし） |
...
> **2026-09-20 追記**: 予定を入れ替え、v0.6.0 は Issue #6（`normalizeLawNum` + `lookupByLawNum` の正規化照合 + `isValidLawId` の e-Gov 実データ準拠）で出した。§3-1 の `expandToFormalNames` は v0.7.0 以降に送る。§3-2 は v0.6.0 で完了（e-Gov 法令 API v2 で全 9,569 件を取得して確定。CHANGELOG 0.6.0 参照）。§4 の `normalizeLawNum` も同時に解消。

---

## 1. v0.5.3（文書作成時の呼称は v0.5.1）— verify-law-ids の完成と CI 化
...
```mermaid
flowchart LR
    A[v0.5.3<br/>verify 本実装+workflow+不整合修正] --> B[v0.5.x<br/>Track4 増強 随時]
