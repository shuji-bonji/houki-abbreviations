# v0.5.2 / v0.6.0 計画: verify workflow 化 + 辞書増強 + synonym 展開

作成日: 2026-07-19 / 現行: v0.5.1

> **2026-09-12 に版番号を 1 つ繰り下げました。** v0.5.1 は JSDoc の `@since` / `@group` 追加と README の誤り修正（公開 API 変更なし）に使ったため、本文書が v0.5.1 と呼んでいる verify-law-ids の実装は **v0.5.2** になります。以下の見出しは作成時のまま残しています。

## 0. 版付け方針（RELEASE.md の運用実績に準拠）

| 版 | 内容 | 種別 |
|---|---|---|
| ~~v0.5.1~~ | ~~verify-law-ids 本実装~~ → **v0.5.2 に繰り下げ**（v0.5.1 は JSDoc + README の修正に使用） | patch |
| **v0.5.2** | verify-law-ids 本実装 + workflow 新設 + 既知の不整合修正 | patch（公開 API 変更なし） |
