...
1. **数字でない語の中の漢数字も算用数字になる。** 漢数字の並びを場所を問わず変換するので、地名や語の一部も変わる。`normalizeLawNum('千葉県条例第一号')` は `'1000葉県条例第1号'`、`normalizeLawNum('三重県')` は `'3重県'`、`normalizeLawNum('平成十五年一般法律第三号')` は `'平成15年1般法律第3号'`。v0.6.0 の辞書で `law_num` を持つ 9 件（`昭和四十年法律第三十三号` など）はこの形を含まないが、条例や地名を含む番号を渡すと元の語が読めなくなる。両方に通して比べる用途では一致は保たれる。年と番号の位置だけを変換するかどうかを人が決める。
2. **読めない漢数字の並びはそのまま残す。** JSDoc は「読めない並びはそのまま残す」と書き、`normalizeLawNum('昭和十十年法律第一号')` は `'昭和十十年法律第1号'`。テストが無い。ID を振るのは受入テストを書いてから。
3. **`元年` は場所を問わず `1年` になる。** `normalizeLawNum('令和 元 年法律第一号')` は空白を取り除いたあとに変換するので `'令和1年法律第1号'`。テストが無い。ID を振るのは受入テストを書いてから。
4. **ダッシュ類のうちテストがあるのは `―`（U+2015）だけ。** `‐` `‑` `–` `—` `−` も `-` になる（`normalizeLawNum('昭和二十四年人事院規則一‑一')` は `'昭和24年人事院規則1-1'`）が、テストが無い。また JSDoc の列挙には `‑`（U+2011）が無く、実装とずれている。罫線 `─`（U+2500）と長音 `ー` は変換しない（`'…一─一'` は `'…1─1'`）。ID を振るのは受入テストを書いてから。
5. **桁数の大きい算用数字で値が変わる。** 先頭の 0 を取るときに数値を経由するので、17 桁以上の番号は丸められる。`normalizeLawNum('昭和25年法律第12345678901234567890号')` は `'昭和25年法律第12345678901234567000号'`。実在の法令番号には無い桁数だが、入力の数字が書き換わる。人が決める。
6. **`null` / `undefined` を渡したとき。** JSDoc のとおり `''` を返す（`normalizeLawNum(null)` は `''`）。テストが無い。ID を振るのは受入テストを書いてから。
