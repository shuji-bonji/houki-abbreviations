...
1. **同じ一致を 2 件返す。** 1 件のエントリの中で `abbr`・`formal`・`aliases` に同じ値が重ねて登録されていると、その値の一致を同じ位置・同じ長さで 2 件返す。長さが同じなので `preferLonger: true`（既定）でも除かれない。v0.6.0 の同梱辞書では 66 件のエントリがこれに当たる。例: `酒税法` → `[{ matchedKey: "酒税法", position: 0 }, { matchedKey: "酒税法", position: 0 }]`（エントリはどちらも `酒税法`）。`民法の解釈` → `民法` の一致を 2 件（エントリはどちらも `民`）。`消費税法基本通達の取扱い` → `消費税法基本通達` の一致を 2 件。`dedupe: true` を渡せば 1 件になる。重ねた辞書の側を直すか、この関数で同じ一致を 1 件にするかを決める（validate_all_entries の未決 4）。
2. **2 つの法令名にまたがる短い一致を返す。** 一致の範囲がほかの一致の範囲に「すっぽり入る」ときだけ除くので、2 つの一致にまたがる短い一致は残る。例: `消費税法法人税法` → `消費税法`（位置 0）、`法法`（位置 3。`消費税法` の `法` と `法人税法` の `法`）、`法人税法`（位置 4）の 3 件。`法法` は文中に書かれた略称ではない。除くかを決める。
3. **全角・半角の表記ゆれを吸収しない。** `PL法` は `製造物責任法` の一致を返すが、`ＰＬ法`（全角の `ＰＬ`）は `[]`。このライブラリの `normalizeJpText`（`ＰＬ法` → `PL法`）を通してから探すかを決める。
