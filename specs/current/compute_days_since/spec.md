...
1. **ISO 8601 以外の書き方も受け付ける。** 引数は ISO 8601 形式と書いているが、JavaScript の日付解釈が受け付ける書き方ならそのまま数える。`nowMs` が `2026-05-08T00:00:00Z` のとき、`"2026/05/07"` → `1`（TZ=Asia/Tokyo）、`"May 7, 2026"` → `1`。ISO 8601 以外を 003 と同じく 0 にするか、今のまま受け付けるかを決める。
2. **時差の書かれていない時刻は、実行環境のタイムゾーンで結果が変わる。** `nowMs` が `2026-05-08T00:00:00Z` のとき、`fetchedAt: "2026-05-07T08:00:00"` は TZ=UTC で `0`、TZ=Asia/Tokyo で `1`。日付だけの `"2026-05-07"` は UTC として扱われ、どちらでも `1`。時差の無い時刻を UTC とみなすか、今のままにするかを決める。
3. **存在しない日付が繰り上がって数えられる。** `fetchedAt: "2026-02-30T00:00:00Z"`、`nowMs` が `2026-05-08T00:00:00Z` → `67`（3 月 2 日として数える）。003 と同じく 0 にするかを決める。
4. **解釈できない文字列や今より後の時刻を `judgeStaleness` に渡すと `fresh` になる。** 002・003 のとおり 0 を返すので、そのまま `judgeStaleness` に渡すと、取得時刻が壊れていても `fresh` と判定される。例: `judgeStaleness(computeDaysSince("not-a-date"))` → `"fresh"`。0 と区別できる値を返すか、呼び出し側の責任のままにするかを決める。
5. **`nowMs` に `NaN` を渡すと `NaN` を返す。** `computeDaysSince("2026-05-07T00:00:00Z", NaN)` → `NaN`。戻り値は 0 以上の整数としているが、この場合は外れる。`judgeStaleness(NaN)` は `"outdated"`。`nowMs` の検査をするかを決める。
