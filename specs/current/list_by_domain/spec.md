...
1. **README の件数が実際と違う。** README の使い方の例は `listByDomain('tax'); // 26 件` と書いているが、v0.6.0 では 35 件を返す（JSDoc の例は 35 件で合っている）。README を直すのか、件数を例に書かないようにするのかを決める。
2. **返すエントリは凍結されておらず、書き換えると辞書に残る。** 配列は呼ぶたびに新しく作られるが、要素のエントリは辞書のエントリそのもので凍結されていない。`listByDomain('tax')[0].formal = 'X'` とすると、以後の `resolveAbbreviation('所法')` の `formal` も `'X'` になる。`resolveAbbreviation` の未決 2 と同じ項目。
