...
2. **位ごとの長い並びで値が正確でなくなる。** 17 桁以上になると数値の精度を超え、`kanjiToNumber('一'.repeat(20))`（`一` が 20 字）は `11111111111111110000` を返す（正しくは 11111111111111111111）。`null` にするか、桁数に上限を置くかを人が決める。
