...
1. **Issue #6 の完了条件のテストが `null` 同士で通っている。** `src/normalize.test.ts` のテスト「Issue #6 の完了条件: 昭和二十五年法律第百三十七号 と 昭和25年法律第137号 が同じ結果」は、2 つの呼び出しの結果が等しいことだけを確かめている。v0.6.0 の辞書には `昭和二十五年法律第百三十七号` のエントリが無く、`lookupByLawNum('昭和二十五年法律第百三十七号')` も `lookupByLawNum('昭和25年法律第137号')` も `null` を返すので、正規化が働かなくてもこのテストは通る。このテストには ID を付けていない。辞書にある法令番号に変えるか、`null` でないことも確かめるかを決める。
2. **`law_num` を持たないエントリを対象にしないことのテスト。** テスト名「law_num 未設定エントリは引けない」の中身は `lookupByLawNum(fixtures, '')` が `null` になることを確かめているだけで、`law_num` を持たないエントリ（fixtures の `労基法`）を引こうとしていない。このテストには SPEC-ABBR-LOOKUP-BY-LAW-NUM-006 を付けた。
...
5. **返すエントリは辞書のオブジェクトそのもの。** 返したエントリは `abbreviationEntries` の要素と同じオブジェクトで、凍結されていない。呼び出し側が書き換えると辞書そのものが変わる（`lookupByLawId` の未決 3 と同じ。例はそちら）。ライブラリ全体で決める。
