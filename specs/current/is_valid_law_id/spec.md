...
1. **先頭 3 桁（元号と年）の値を確かめない。** 数字 3 桁なら何でも受け付ける。`000AC0000000000` と `999AC9999999999` は `true`。憲法の形も先頭 3 桁を問わず、`363CONSTITUTION` は `true`（e-Gov にある憲法は `321CONSTITUTION` の 1 件だけ）。形の判定としてこれでよいか、元号の桁（1〜5）や憲法の `321` まで見るかを決める。
2. **M の形の府省コードの 1 文字目が、実装の注記と違う範囲まで通る。** 実装の注記には「`M` の次の 1 文字は 1〜6」とあるが、実際には 0〜9 と `A`〜`F` のどれでも通る。`340M00000040011` と `340MF0000040011` は `true`。注記に合わせて狭めるか、注記を直すかを決める。
