...
describe('normalizeJpText()', () => {
  it('SPEC-ABBR-NORMALIZE-JP-TEXT-007 returns empty string for empty / falsy input', () => {
    expect(normalizeJpText('')).toBe('');
  });

  it('SPEC-ABBR-NORMALIZE-JP-TEXT-006 trims surrounding whitespace', () => {
    expect(normalizeJpText('  消法  ')).toBe('消法');
  });

  it('SPEC-ABBR-NORMALIZE-JP-TEXT-001 converts full-width digits to half-width', () => {
    expect(normalizeJpText('１８３')).toBe('183');
    expect(normalizeJpText('０１２３４５６７８９')).toBe('0123456789');
  });

  it('SPEC-ABBR-NORMALIZE-JP-TEXT-003 converts full-width hyphen to half-width hyphen', () => {
    expect(normalizeJpText('１８３－２')).toBe('183-2');
    expect(normalizeJpText('第２－３条')).toBe('第2-3条');
  });

  it('SPEC-ABBR-NORMALIZE-JP-TEXT-004 converts full-width tilde variants to half-width tilde', () => {
  ...
  it('SPEC-ABBR-NORMALIZE-JP-TEXT-005 converts full-width space to half-width space', () => {
    expect(normalizeJpText('消　法')).toBe('消 法');
  });

  it('SPEC-ABBR-NORMALIZE-JP-TEXT-002 converts full-width ASCII letters but preserves case', () => {
  ...
  it('SPEC-ABBR-NORMALIZE-JP-TEXT-008 preserves nakaguro (中黒)', () => {
    expect(normalizeJpText('１の３・１の４共-1')).toBe('1の3・1の4共-1');
  });

  it('SPEC-ABBR-NORMALIZE-JP-TEXT-008 preserves Japanese characters (kana, kanji)', () => {
    expect(normalizeJpText('消費税法基本通達')).toBe('消費税法基本通達');
    expect(normalizeJpText('カタカナひらがな漢字')).toBe('カタカナひらがな漢字');
  });

  it('SPEC-ABBR-NORMALIZE-JP-TEXT-008 preserves halfwidth kana (no conversion)', () => {
    // 半角カナはそのまま
    expect(normalizeJpText('ｱｲｳｴｵ')).toBe('ｱｲｳｴｵ');
  });

  it('SPEC-ABBR-NORMALIZE-JP-TEXT-008 preserves common Japanese clause-number markers', () => {
    // 共・の・条・項・章・節・款 などの構造マーカーは保持
    expect(normalizeJpText('１の３共-1')).toBe('1の3共-1');
    expect(normalizeJpText('第１条第２項第３号')).toBe('第1条第2項第3号');
  });

  it('SPEC-ABBR-NORMALIZE-JP-TEXT-001 SPEC-ABBR-NORMALIZE-JP-TEXT-002 SPEC-ABBR-NORMALIZE-JP-TEXT-003 SPEC-ABBR-NORMALIZE-JP-TEXT-005 SPEC-ABBR-NORMALIZE-JP-TEXT-006 combines all transformations in one pass', () => {
    expect(normalizeJpText('  ＰＬ法１８３－２ 消　法  ')).toBe('PL法183-2 消 法');
  });
});

describe('normalizeSearchQuery()', () => {
  it('SPEC-ABBR-NORMALIZE-SEARCH-QUERY-006 returns empty string for empty input', () => {
  ...
  it('SPEC-ABBR-NORMALIZE-SEARCH-QUERY-002 lowercases ASCII uppercase letters', () => {
    expect(normalizeSearchQuery('PL法')).toBe('pl法');
    expect(normalizeSearchQuery('ＰＬ法')).toBe('pl法');
  });

  it('SPEC-ABBR-NORMALIZE-SEARCH-QUERY-003 preserves Japanese characters during lowercase', () => {
    // 大文字小文字変換は ASCII のみ。漢字・ひらがな・カタカナは無変化。
    expect(normalizeSearchQuery('消費税法')).toBe('消費税法');
    expect(normalizeSearchQuery('カタカナ')).toBe('カタカナ');
  });

  it('SPEC-ABBR-NORMALIZE-SEARCH-QUERY-004 collapses consecutive whitespace into a single space', () => {
    expect(normalizeSearchQuery('消    法')).toBe('消 法');
    expect(normalizeSearchQuery('a   b\tc\n d')).toBe('a b c d');
  });

  it('SPEC-ABBR-NORMALIZE-SEARCH-QUERY-005 trims and collapses combined', () => {
    expect(normalizeSearchQuery('  消    法  ')).toBe('消 法');
  });
});

describe('resolveAbbreviation() with { normalize: true }', () => {
  it('SPEC-ABBR-RESOLVE-ABBREVIATION-001 SPEC-ABBR-RESOLVE-ABBREVIATION-004 SPEC-ABBR-RESOLVE-ABBREVIATION-005 default behavior (no options) is unchanged from v0.2.0', () => {
    // 後方互換性のための回帰テスト
  ...
  it('SPEC-ABBR-RESOLVE-ABBREVIATION-006 default ({ normalize: false }) does not absorb full-width variations', () => {
  ...
  it('SPEC-ABBR-RESOLVE-ABBREVIATION-009 { normalize: true } absorbs full-width digits', () => {
  ...
  it('SPEC-ABBR-RESOLVE-ABBREVIATION-007 { normalize: true } absorbs full-width ASCII letters', () => {
    // ＰＬ法 (full-width) → PL法 (half-width) → resolves
    const r = resolveAbbreviation('ＰＬ法', { normalize: true });
    expect(r?.formal).toBe('製造物責任法');
  });

  it('SPEC-ABBR-RESOLVE-ABBREVIATION-008 { normalize: true } preserves case sensitivity', () => {
  ...
  it('SPEC-ABBR-RESOLVE-ABBREVIATION-004 { normalize: true } absorbs full-width spaces', () => {
  ...
  it('SPEC-ABBR-RESOLVE-ABBREVIATION-005 { normalize: true } returns null for genuinely unknown names', () => {
  ...
  it('SPEC-ABBR-RESOLVE-ABBREVIATION-009 { normalize: true } still works on aliases', () => {
  ...
  it('SPEC-ABBR-RESOLVE-ABBREVIATION-009 { normalize: true } prefers exact-match over normalized-match (consistency)', () => {
...
describe('kanjiToNumber()', () => {
  it('SPEC-ABBR-KANJI-TO-NUMBER-001 位取りの漢数字を読む', () => {
  ...
  it('SPEC-ABBR-KANJI-TO-NUMBER-002 位ごとの漢数字（単位なし・〇あり）を読む', () => {
  ...
  it('SPEC-ABBR-KANJI-TO-NUMBER-003 SPEC-ABBR-KANJI-TO-NUMBER-004 読めない並びは null', () => {
...
describe('normalizeLawNum()', () => {
  it('SPEC-ABBR-NORMALIZE-LAW-NUM-001 SPEC-ABBR-NORMALIZE-LAW-NUM-002 SPEC-ABBR-NORMALIZE-LAW-NUM-003 漢数字・算用数字・全角数字を同じ文字列にする', () => {
  ...
  it('SPEC-ABBR-NORMALIZE-LAW-NUM-004 元年は 1 年にする', () => {
    expect(normalizeLawNum('令和元年法律第一号')).toBe('令和1年法律第1号');
    expect(normalizeLawNum('平成元年法律第四十二号')).toBe('平成1年法律第42号');
  });

  it('SPEC-ABBR-NORMALIZE-LAW-NUM-005 SPEC-ABBR-NORMALIZE-LAW-NUM-006 番号の無い法令番号・憲法・人事院規則の形もそのまま揃える', () => {
  ...
  it('SPEC-ABBR-NORMALIZE-LAW-NUM-007 共同省令の長い種別名は数字以外を変えない', () => {
    expect(
      normalizeLawNum(
  ...
  it('SPEC-ABBR-NORMALIZE-LAW-NUM-008 吸収しないもの: 元号の別表記、第・号の省略', () => {
  ...
  it('SPEC-ABBR-NORMALIZE-LAW-NUM-009 空文字は空文字', () => {
    expect(normalizeLawNum('')).toBe('');
  });
});

describe('lookupByLawNum() — 算用数字でも引ける', () => {
  it('SPEC-ABBR-LOOKUP-BY-LAW-NUM-001 SPEC-ABBR-LOOKUP-BY-LAW-NUM-002 漢数字と算用数字で同じエントリを返す', () => {
